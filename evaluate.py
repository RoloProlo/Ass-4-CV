...
def compute_ap(class_id, predictions, ground_truths, iou_threshold=0.2):
    tp = []
    scores = []
    gt_all = [(cid, box) for cid, boxes in ground_truths.items() for box in boxes]
    matched = set()

    for pred_class, preds in predictions.items():
        for pred_box, score in sorted(preds, key=lambda x: -x[1]):
            scores.append(score)
            match_found = False
            for idx, (gt_class, gt_box) in enumerate(gt_all):
    ...
    tp_cum = np.cumsum(tp)
    fp_cum = np.cumsum(fp)
    recalls = tp_cum / (len(ground_truths[class_id]) + 1e-6)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("models/1best_model.pth", map_location=torch.device('cpu')))
    model.eval()
